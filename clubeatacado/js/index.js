    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            if (data.message) {
                ...
                appendProduto('#listPromocao', data.response.sucesso.message.promocoes);
    ...
    if (produto.length > 0) {
        for (var i = 0; i < produto.length; i++) {
            ...
            if (produto[i].IN_PROMOCAO == 1) {
                card_produto = card_produto + '<div class="container" style="position:absolute; border-radius: 10px; width:106px;height:32px;background-color:#FFE500;z-index: 1;">' +
                '<p class="subtitle is-danger is-6" style="position:absolute; top:8px;right:15px;width:75px;font-weight:bold;">OFERTAS</p>' +
                '</div>';
            }
            card_produto = card_produto + '<img src="css\\img\\rectangle-2.png" style="border-radius: 8px;" alt="Placeholder image">' +
            '</figure>' +
            '</div>' +
            '<div class="card-content" style="position:absolute;top:100px;left:14px">' +
            '<p class="imagem-titulo">'+produto[i].DS_PRODUTO+'+</p>' +
            '<p style="color: red;" class="imagem-sub-titulo">De <strike>R$ '+produto[i].VL_PRODUTO_ANTIGO+'</strike> Por <span style="font-size: 20px;">R$ '+produto[i].VL_PRODUTO_UNITARIO+'</span></p>' +
            ...
            if (produto[i].FLAG_FAVORITO == 1) {
                card_produto = card_produto + '<a onclick="favoritarAnuncio('+produto[i].ID_ANUNCIO_PRODUTO+')" class="button gradiente"><img id="imgFavorito'+produto[i].ID_ANUNCIO_PRODUTO+'" style="height:27x; width:30px" src="css\\img\\16.png"></a>';
            } else {
                card_produto = card_produto + '<a onclick="favoritarAnuncio('+produto[i].ID_ANUNCIO_PRODUTO+')" class="button gradiente"><img id="imgFavorito'+produto[i].ID_ANUNCIO_PRODUTO+'" style="height:27x; width:30px" src="css\\img\\icons8-favorite-60.png"></a>';
            }
            '</div>' +
            '</div>' +
            '</div>' +
            ...
            // if (produto[i].FLAG_FAVORITO == 1) {
            //     card_produto = card_produto + '<a onclick="favoritarAnuncio(' + produto[i].ID_ANUNCIO_PRODUTO + ')" class="button gradiente">' +
            //         '<img id="imgFavorito' + produto[i].ID_ANUNCIO_PRODUTO + '" style="height:27x; width:30px" src="css\\img\\16.png">' +
            //         '</a>';
            // } else {
            //     card_produto = card_produto + '<a onclick="favoritarAnuncio(' + produto[i].ID_ANUNCIO_PRODUTO + ')" class="button gradiente">' +
            //         '<img id="imgFavorito' + produto[i].ID_ANUNCIO_PRODUTO + '" style="height:27x; width:30px" src="css\\img\\icons8-favorite-60.png">' +
            //         '</a>';
            // }


            // if (produto[i].DS_FOTO_ANUNCIO_PRODUTO != 'http://testetendering.myappnow.com.br/clubeatacado/images/anuncios/104/plasticos.png') {
            ...
            // } else {
            //     card_produto = card_produto + '</div>' +
            //         '<img src="css\\img\\tenderingAnuncio.jpeg" style="filter: brightness(60%);border-radius: 8px;width: 350px;height: 158px;" alt="Placeholder image">' +
            //         '</figure>' +
            //         '</div>';
            // }

            // card_produto = card_produto + '<div class="card-content" style="position:absolute;top:100px;left:14px">' +
            //     '<p class="subtitle is-bold is-6 is-left" style="color: white;margin-bottom: 0.5rem;">' + produto[i].DS_PRODUTO + '</p>';
            // if (produto[i].IN_PROMOCAO == 1) {
            //     if (produto[i].VL_PRODUTO_ANTIGO != null) {
        ...
        success: function (data, textStatus, jqXHR) {
            if (data.message) {
                ...
                if (data.response.sucesso.message.returnFavorito == 1) {
                    $("#imgFavorito" + idAnuncioProduto).attr("src", "css\\img\\16.png");
                } else if (data.response.sucesso.message.returnFavorito == 0) {
                    $("#imgFavorito" + idAnuncioProduto).attr("src", "css\\img\\icons8-favorite-60.png");
                }
