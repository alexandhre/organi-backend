...
class AnuncioController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $anuncio = Anuncio::novoAnuncio($data);
...
//            'capacidade' => $credentials['capacidade'],
        ...
        $cor = Cor::cor($usuario['input'][9], $anuncio);
            ...
            for ($i = 0; $i < 5; $i++) {
                ...
                //adicionar pasta usuario
                if($file->moveDirectory('C:/Inetpub/vhosts/myappnow.com.br/atacado.club/clubeatacado/images/resource/tmp/anuncio/'.$credentials['fotos'][$i],  "C:/Inetpub/vhosts/myappnow.com.br/atacado.club/clubeatacado/images/anuncio/" . $anuncio."/".$credentials['fotos'][$i])){
                    //$file->moveDirectory('images\\resource\\tmp\\empresa\\'.$data[$i],  "images\\empresas\\".$usuarioId."\\".$data[$i])

                    $foto = FotoAnuncio::updatefoto($anuncio, $credentials['fotos'][$i]);
...
//                \File::delete('images\\resource\\tmp\\empresa\\'.$data[$i]);
            ...
            for ($j = 0; $j < 5; $j++) {
                ...
                //adicionar pasta usuario
                if($file->moveDirectory('C:/Inetpub/vhosts/myappnow.com.br/atacado.club/clubeatacado/images/resource/tmp/anuncio/'.$credentials['certificados'][$j],  "C:/Inetpub/vhosts/myappnow.com.br/atacado.club/clubeatacado/images/anuncio/". $anuncio."/cetificado".$j.".".$extencao)) {
                    //$file->moveDirectory('images\\resource\\tmp\\empresa\\'.$data[$j],  "images\\empresas\\".$usuarioId."\\".$data[$j])
                    $id = Atacadista::findId(session()->all()['id']);

                    $foto = Certificado::updatefoto($id, "cetificado".$j.".".$extencao, $anuncio);
//                \File::delete('images\\resource\\tmp\\empresa\\'.$data[$j]);
    ...
    public function uploadfotoanucio(Request $request){
        ...
        if ($request->hasFile('myFile')) {
            ...
            if(($image != null) &&($tamanho[0] < 2*$tamanho[1])&&($tamanho[1] < 2*$tamanho[0])) {
                $destinationPath = ('C:/Inetpub/vhosts/myappnow.com.br/atacado.club/clubeatacado/images/resource/tmp/anuncio/');
    ...
    public function anuncioDetalhe($id){

            $anuncios = Anuncio::anuncio($id);
...
//        $anuncio = Anuncio::todasPromocoes();
    ...
    public function anuncie(){
        ...
        if(!key_exists('email',session()->all())) {
           ...
           return (new \App\Http\Controllers\usuario\UsuarioController)->editar();
        }else{
...
//        }
    ...
    public function visita($id){
        ...
        if(!key_exists('email',session()->all())){
                ...
                Visita::created($id, session()->all()['id']);
            }
